...
\author{Adrien~Besson,~\IEEEmembership{Student member}
        Rafael~E.~Carrillo,~\IEEEmembership{Member}
        Dimitris~Perdios,~\IEEEmembership{Student member}
        Marcel~Arditi,~\IEEEmembership{Member},
        Yves~Wiaux,
        Jean-Philippe Thiran,~\IEEEmembership{Senior member}% <-this % stops a space
...
\thanks{Y. Wiaux is with the Institute of Sensors, Signals and Systems, Heriot-Watt University, EH14 4AS Edinburgh, United-Kingdom.}% <-this % stops a space
\thanks{J.-Ph. Thiran is also with the Department of Radiology, University Hospital Center~(CHUV) and University of Lausanne~(UNIL), CH-1011, Lausanne, Switzerland.}}% <-this % stops a space
...
\begin{IEEEkeywords}
Compressed sensing, sparsity, ultrasound imaging
\end{IEEEkeywords}
